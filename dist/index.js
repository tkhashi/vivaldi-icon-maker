    ...
    try {
        ...
        for (const variant of variants) {
            ...
            if (variant.backgroundColor) {
                recolored = addOrUpdateBackgroundRect(recolored, variant.backgroundColor, INACTIVE_CORNER_RADIUS);
            }
...
function transformInactiveColor(color, mix) {
    if (!color || color.trim().toLowerCase() === "none") {
        return color;
    }
    return createInactiveColor(color, mix);
}
function pickPrimaryColor(options) {
...
function buildOutputPath(options, variantName) {
    return path.join(options.outputDir, `${options.outputBaseName}-${variantName}${options.outputExtension}`);
}
...
function parseInactiveMix(value) {
...
function addOrUpdateBackgroundRect(svgContent, color, cornerRadius) {
...
function buildInsetRectFromViewBox(viewBox, color, cornerRadius) {
...
function buildInsetRectWithPercent(color, cornerRadius) {
    const insetRatio = clampRatio(INACTIVE_BACKGROUND_INSET_RATIO);
